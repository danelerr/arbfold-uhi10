...
/// @dev Shared integer arithmetic for both frozen mechanisms.
library CycleMath {
    ...
    /// @dev Closed-form optimum of A*x/(B+C*x)-x after bounded coefficient normalization.
    function _optimalInput(Leg[3] memory legs) private pure returns (uint256) {
        ...
        for (uint256 i = 1; i < 3; ++i) {
            ...
            uint256 composedA = nextA * a;
            uint256 composedB = nextB * b;
            uint256 composedC = nextB * c + GAMMA * a;
            (a, b, c) = _normalize(composedA, composedB, composedC);
        }
    ...
    function _normalize(uint256 a, uint256 b, uint256 c)
        private
        pure
