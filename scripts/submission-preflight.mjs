...
const [readme, page, app, finalSubmission, checklist, rawText, manifestText] = await Promise.all([
  read("README.md"),
  read("app/index.html"),
  read("app/app.js"),
...
const displayedSizes = [...page.matchAll(/data-size="(\d+)"/g)].map((match) => Number(match[1]));
...
check("Wallet bundle is explicitly versioned", page.includes('./app.js?v=receipt-state-1'));
...
if (publicMode) {
  try {
    ...
    check("Public dashboard serves current wallet bundle", publicPage.includes("./app.js?v=receipt-state-1"));
