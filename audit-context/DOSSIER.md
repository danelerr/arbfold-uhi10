...
Límites que siguen fuera de ese PASS: `check:live` no relee pool keys, `isOperator`, igualdad claim/reserva ni backing; no recorre las 28 deployment txs; y sólo imprime el residual actual, sin exigir que sea ≤ `RESIDUAL_THRESHOLD`. El verificador Solidity cubre los primeros estados (`contracts/script/VerifyArbFoldDeployment.s.sol:L56-L140`) y también pasó en esta revalidación. La procedencia de los ocho contratos activos se comprueba por separado con `npm run check:sources`; el PoolManager oficial es una dependencia externa y no se presenta como fuente propia de ARBFOLD.
